...
export default function AdminSettingsTab() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="bg-[#15171c] p-6 rounded-3xl border border-white/5">
        <h3 className="text-xl font-bold text-white mb-1">Admin Settings</h3>
        <p className="text-sm text-muted-foreground">Manage your admin notifications and security preferences.</p>
      </div>
        ...
        <div className="bg-[#15171c] border border-white/5 rounded-3xl overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-white flex items-center gap-2">
                  <Users size={16} className="text-muted-foreground"/> New User Notifications
                </h3>
                <p className="text-sm text-muted-foreground mt-1">Receive alerts when new users register on the platform.</p>
              </div>
              <Toggle enabled={settings.new_user_notifications} onChange={(v) => handleToggle('new_user_notifications', v)} />
            </div>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-white flex items-center gap-2">
              ...
              <Toggle enabled={settings.mentorship_notifications} onChange={(v) => handleToggle('mentorship_notifications', v)} />
            </div>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-white flex items-center gap-2">
                  <Share2 size={16} className="text-muted-foreground"/> Referral Notifications
                </h3>
                <p className="text-sm text-muted-foreground mt-1">Get notified about new referrals posted across the platform.</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-white flex items-center gap-2">
                  <Briefcase size={16} className="text-muted-foreground"/> Application Notifications
                </h3>
                <p className="text-sm text-muted-foreground mt-1">Updates when application statuses change on the platform.</p>
              </div>
        ...
        <div className="bg-[#15171c] border border-white/5 rounded-3xl overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-center justify-between bg-white/5 p-4 rounded-2xl border border-white/10">
              ...
              {isOAuth ? (
                <span className="text-xs font-semibold px-3 py-1.5 bg-white/10 text-muted-foreground rounded-lg">
                  Password managed by {userProvider === 'google' ? 'Google' : 'LinkedIn'}
                </span>
              ) : (
                <button 
                  onClick={() => {
                  ...
                  className="text-sm font-bold text-primary hover:text-white transition-colors flex items-center gap-1.5 bg-primary/10 px-4 py-2 rounded-xl border border-primary/20"
                >
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Sign out of your admin session securely.
                  </p>
                </div>
                <button 
                  onClick={() => { logout(); navigate('/login'); }}
                  className="whitespace-nowrap px-6 py-2.5 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl transition-all flex items-center gap-2"
                >
                  <LogOut size={16} /> Logout
                </button>
              </div>

              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-2xl bg-red-500/5 border border-red-500/10">
                <div>
                  <h3 className="font-semibold text-white">Delete Account</h3>
                  <p className="text-sm text-red-400/80 mt-1 max-w-lg">
                    Permanently remove your admin account. This action is not reversible.
                  </p>
                </div>
                <button 
                  onClick={() => setShowDeleteModal(true)}
                  className="whitespace-nowrap px-6 py-2.5 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 font-bold border border-red-500/20 rounded-xl transition-all flex items-center gap-2"
                >
        ...
        {showDeleteModal && (
          <>
            ...
            <motion.div 
              ...
              className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-[#1c1f26] border border-red-500/20 rounded-3xl p-8 z-50 shadow-2xl"
            >
