...
import {
  LogOut, LayoutDashboard, Settings, DollarSign, Award,
  Users, TrendingUp, RefreshCw, Plus, Heart,
...
export default function DonorDashboard() {
  ...
  // Mock data
  const stats = { scholarshipsFunded: 3, totalDonated: '$15,000', studentsHelped: 12 };
  ...
  const navItems = [
    ...
    { id: 'scholarships', label: 'My Scholarships', icon: Award },
    ...
    <div className="flex h-screen bg-[#09090b] text-white overflow-hidden">
      ...
      <aside className="w-64 shrink-0 flex flex-col border-r border-white/10 bg-[#0f1117] p-5">
        <div className="flex items-center gap-3 mb-10">
          <div
            className="w-10 h-10 rounded-xl overflow-hidden border border-yellow-500/30 bg-yellow-500/10 cursor-pointer flex items-center justify-center text-lg font-bold text-yellow-400"
            onClick={() => fileInputRef.current?.click()}
          >
            {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" /> : (user?.name?.[0]?.toUpperCase() || 'D')}
          </div>
          <input ref={fileInputRef} type="file" accept="image/*" className="hidden" />
          <div>
            <p className="font-bold text-sm text-white truncate max-w-[140px]">{user?.name}</p>
            <span className="text-xs font-bold text-yellow-400 bg-yellow-500/10 px-2 py-0.5 rounded border border-yellow-500/20">Donor</span>
          </div>
        </div>
        ...
        <div className="p-8 space-y-8 max-w-5xl mx-auto">
          {activeTab === 'overview' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
              {/* Welcome Banner */}
              <div className="rounded-3xl border border-yellow-500/20 bg-gradient-to-br from-yellow-500/10 via-transparent to-transparent p-8 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-yellow-500/5 rounded-full blur-3xl pointer-events-none" />
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-yellow-400 text-sm font-semibold mb-1">👋 Welcome to Donor Portal</p>
                    <h2 className="text-3xl font-bold text-white mb-2">{user?.name}</h2>
                    <p className="text-muted-foreground">Thank you for empowering the next generation of students.</p>
                  </div>
                  <button onClick={() => setActiveTab('scholarships')} className="flex items-center gap-2 text-sm font-semibold text-yellow-400 hover:text-yellow-300 bg-yellow-500/10 border border-yellow-500/20 px-4 py-2 rounded-xl hover:bg-yellow-500/20 transition-all">
                    <Heart size={14} /> Fund a Scholarship
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-6">
                {[
                  { label: 'Scholarships Funded', value: stats.scholarshipsFunded, icon: Award, color: 'text-yellow-400', bg: 'from-yellow-500/10' },
                  { label: 'Total Donated', value: stats.totalDonated, icon: DollarSign, color: 'text-emerald-400', bg: 'from-emerald-500/10' },
                  { label: 'Students Helped', value: stats.studentsHelped, icon: Users, color: 'text-blue-400', bg: 'from-blue-500/10' },
                ].map((s, i) => (
